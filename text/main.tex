...
\begin{listing}[H]
\begin{minted}[breaklines,fontsize=\scriptsize]{rust}
pub func get_supply_sales() -> u64 {
    return length(own_contract.sales);
}

pub func get_supply_by_owner_id(account_id) -> u64 {
    owner_id = own_contract.by_owner_id.get(account_id);
    return length(owner_id);
}

pub func get_sales_by_owner_id(account_id, from_index, limit) -> Vec<Sale> {
...
pub func get_supply_by_nft_contract_id(nft_contract_id) -> u64 {
    nft_contract_id = own_contract.by_nft_contract_id.get(nft_contract_id);
    return length(nft_contract_id);
}

pub func get_sales_by_nft_contract_id(nft_contract_id, from_index, limit) -> Vec<Sale> {
...
\end{minted}
\caption{Marketplace contract enumaration}
...
\begin{figure}
    ...
    \subfloat[Список NFT на маркеплейсе]{
        ...
        \includegraphics[height=80mm]{fig/buynftmarket.png}
    }
    \subfloat[Список NFT пользователя у которого можно купить NFT]{
        \label{fig.nftonsaleuser}
        \includegraphics[height=80mm]{fig/nftonsaleuser.png}
    }\\
    \subfloat[Подтверждение покупки]{
        \label{fig.buyconfirm}
        \includegraphics[height=90mm]{fig/buyconfirm.png}
    }
    \caption{Покупка NFT}
\end{figure}
...
\begin{figure}
    ...
    \subfloat[Конструктор NFT]{
        ...
        \includegraphics[height=80mm]{fig/createnftconstructor.png}
    }\\
    \subfloat[Законченное NFT]{
        \label{fig.createnftconstructordone}
        \includegraphics[height=90mm]{fig/createnftconstructordone.png}
    }
    \subfloat[Подтверждение минта]{
        \label{fig.createnftconstructorconfirm}
        \includegraphics[height=90mm]{fig/createnftconstructorconfirm.png}
    }
    \caption{Минт NFT}
\end{figure}
...
\begin{figure}
    ...
    \subfloat[Сообщение о переходе в личные сообщения]{
        ...
        \includegraphics[height=80mm]{fig/mynftlistcansale.png}
    }
    \subfloat[Конструктор NFT]{
        \label{fig.saleconfirm}
        \includegraphics[height=80mm]{fig/saleconfirm.png}
    }
