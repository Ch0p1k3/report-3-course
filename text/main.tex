...
\subsection{Деплой бота}
	Чтобы развернуть наш проект в интернете используется Alibaba cloud ecs(Elastic Compute Service). Для деплоя используется прокирование с помощью nginx. 
	\begin{figure}[H]
		\centering
		\includegraphics[height=30mm]{fig/nginxganservice.png}
